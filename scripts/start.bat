...
:: Per-VM lock on the LOCAL C: drive (NOT %SHARED%). %SHARED% is the same
:: folder on both VMs, so a shared lock here deadlocked the fleet on
:: 2026-07-20 when both booted at once. C: is a separate disk per VM.
set "LOCKDIR=%SystemDrive%\Windows\Temp\mt5-start.running"
...
:: Exit 0 = acquired, exit 10 = a live instance from THIS boot holds it.
:: On any other exit code, fall back to a plain mkdir lock.
